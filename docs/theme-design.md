...
| 字体族 | `Inter, ui-sans-serif, system-ui, sans-serif` | 系统栈（`-apple-system, BlinkMacSystemFont, 'Segoe UI', …`，见 `frontend/src/style.css`） | **风险低**：两个都是无衬线栈，切换只影响字形 |
| 控件高度 | 无（32px 是 antd 默认值，不在契约里） | 我们自己定 | 无处可映射 |

据此的建议（**这是产品判断，不是技术结论，需你定**）：

- **建议做**：间距（8/12/16/18/24 一对一，改动可预测）、字体族（低风险）。
- **建议做但要知道代价**：字号只映射有 1px 内对应关系的四档（xs/sm/base/md），`lg/xl/2xl` 保持我们自己的值。代价是 base 由 13px 变 13.6px、lg(14px) 与 base(13.6px) 只差 0.4px，卡标题与正文的层级会变紧。
