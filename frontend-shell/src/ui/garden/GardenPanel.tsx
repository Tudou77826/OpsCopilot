  ...
  return (
    <div role="dialog" aria-label={`${meta?.name ?? specimen.speciesId} 收藏详情`} className="garden-collection-card">
      <div className="garden-collection-figure">
        {image
          ? <img className="garden-collection-image" src={image.src} alt={meta?.name} />
          : pack?.art === 'image' ? <span className="garden-art-unavailable">形象待配图</span>
          : <FloraSpecimen speciesId={specimen.speciesId} level={specimen.level} shiny={specimen.shiny} size={120} label={meta?.name} variant="grounded" />}
      </div>
      <div className="garden-collection-body">
        ...
        <div className="garden-collection-stats">
          <div className="garden-progress-row">
            等级 {specimen.level} / {MAX_LEVEL}{specimen.level >= MAX_LEVEL ? `（${pack?.presentation?.maxLevel ?? '满级'}）` : ''} · 本级成长 {Math.round(levelProgress(specimen) * 100)}%
...
export default function GardenPanel({ host, isOpen = true, height = 260, packId = DEFAULT_PACK_ID }: GardenPanelProps) {
  ...
  const dismissPending = (at: string) => {
    void host?.dismiss(at).then(() => {
      setDismissError('')
      setSnapshot(current => current ? { ...current, pending: (current.pending ?? []).filter(item => item.at !== at) } : current)
    }).catch((reason: Error) => setDismissError(`标记已读失败：${reason.message}`))
  }
  ...
  return (
    <div className={`garden-root${pack?.art === 'image' ? ' garden-picture-panel' : ''}`} style={{ height, border: '1px solid var(--g-chip-border)' }}>
      ...
      {specimens.length === 0 ? (
        <>
          {pack?.art !== 'image' ? <Backdrop /> : null}
          <div className="garden-empty" role="status">
            空{pack?.presentation?.title ?? '花园'} —— {pack?.presentation?.empty ?? '完成一次有效操作，开启你的收藏。'}
          </div>
        </>
      ) : null}
      <div className="garden-chip-row">
        <span className="garden-chip" style={{ fontWeight: 600 }}>
          {pack?.presentation?.title ?? '花园'}
          <span className="garden-chip-muted">
            {snapshot ? `等级 ${snapshot.gardenLevel} · ${specimens.length} ${pack?.presentation?.unit ?? '株'}` : '载入中…'}
          </span>
        </span>
        <span className="garden-chip garden-chip-muted garden-pity-summary" style={{ marginLeft: 'auto' }} title={`${snapshot?.pitySinceShiny ?? 0} / ${PITY_AT}，集满必出闪光`}>
      ...
      {dismissError && <div role="alert" className="garden-message" data-kind="error">{dismissError}</div>}
    </div>
  )
}
