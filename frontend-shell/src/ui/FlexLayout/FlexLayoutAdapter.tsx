...
import { TimestampResult } from '../timestampParser';
import { TerminalRuntime } from '../ports';
...
const FlexLayoutAdapter: React.FC<FlexLayoutAdapterProps> = ({
    ...
    // 外部上下文（例如 Task 详情）指定终端后，选中已存在的对应 Tab。
    useEffect(() => {
        if (!activeTerminalId || !model.getNodeById(activeTerminalId)) return;
        model.doAction(Actions.selectTab(activeTerminalId));
    }, [activeTerminalId, model]);
    ...
    const factory = useCallback((node: TabNode) => {
        if (node.getComponent() === 'fileTransfer') {
            return renderFileTransfer?.(activeTerminalId || null, terminals) ?? null;
        }
    ...
    }, [activeTerminalId, terminals, onTerminalData, terminalRefs, completionDelay, terminalConfig, onTerminalFontSizeChange, highlightRules, theme, onSelectionParsed, isBroadcastMode, broadcastIds, onToggleTerminalBroadcast, terminalRuntime, renderTerminal, renderFileTransfer]);
        ...
        >
            ...
            {contextMenu && (
                <ContextMenu
                    ...
                    onOpenFileTransfer={renderFileTransfer ? (id) => { openFileTransferTab(id); setContextMenu(null); } : undefined}
                    onDetach={onDetachTerminal ? (id) => { onDetachTerminal(id); setContextMenu(null); } : undefined}
                    isBroadcastMode={isBroadcastMode}
                    broadcastIds={broadcastIds}
...
const TerminalWrapper: React.FC<TerminalWrapperProps> = ({
    ...
    const isActive = broadcastIds?.includes(terminalId);
    const attachRef = useCallback((value: TerminalRef | null) => {
        if (value) terminalRefs.current.set(terminalId, value);
        else terminalRefs.current.delete(terminalId);
    }, [terminalId, terminalRefs]);
    ...
    return (
        ...
        >
            {renderTerminal ? renderTerminal(terminalId, attachRef) : (
                <TerminalComponent
                    id={terminalId}
            ...
            {isBroadcastMode && (
                <div
                    style={{
                        ...
                        right: '20px',
                        zIndex: 10,
                        backgroundColor: isActive ? 'color-mix(in srgb, var(--success) 90%, transparent)' : 'var(--bg-hover)',
                        color: 'var(--text-on-accent)',
                        padding: '4px 8px',
...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, terminalId, terminals, onCloseTerminal, onRename, onDuplicate, onOpenFileTransfer, onDetach, isBroadcastMode, broadcastIds, onToggleTerminalBroadcast, onStartBroadcastFrom, onClose }) => {
    ...
    return (
        ...
        >
            ...
            {onDetach && (
                <div style={menuItemStyle} onClick={() => { onDetach(terminalId); onClose(); }}>
                    独立窗口打开
                </div>
            )}
            {broadcastProps && (
