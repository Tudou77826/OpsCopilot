...
import { parseTimestamp, TimestampResult } from '../timestampParser';
...
export interface TerminalProps {
    ...
    /** 宿主运行时。共享 UI 不直接访问 Wails 或 sidecar。 */
...
const TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ id, sessionID, onData, completionDelay = 150, terminalConfig, onFontSizeChange, highlightRules, theme = DEFAULT_THEME, onSelectionParsed, runtime = NOOP_RUNTIME }, ref) => {
    ...
    // runtime 走 ref：init effect 依赖不含 runtime（避免宿主 runtime 更新导致终端重建），
    // 补全闭包必须始终读到最新 runtime——sidecar 链 client 就位后 runtime 才补上 getCompletions。
    const runtimeRef = useRef<TerminalRuntime>(runtime);
    ...
    }, [onData, onSelectionParsed, sessionID, completionDelay, terminalConfig, onFontSizeChange, highlightRules, theme, runtime]);
    ...
    const syncSizeToBackend = useCallback(() => {
        ...
        void runtime.resize(sessionIDRef.current, cols, rows);
    }, [runtime]);
    ...
    // 读 runtimeRef 而非闭包 runtime：宿主（如 sidecar）在终端挂载后才补上 getCompletions，
    // init effect 闭包若捕获旧 runtime 会永久静默关闭补全。
    const fetchCompletions = useCallback(async (input: string) => {
        const runtime = runtimeRef.current;
        if (!runtime.getCompletions) {
            return;
        }

        try {
            const data = await runtime.getCompletions(input, input.length);
            if (!data) return;
