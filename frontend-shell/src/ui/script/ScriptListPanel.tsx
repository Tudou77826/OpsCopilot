...
import type { ScriptRuntime, ScriptData, ScriptVariable } from './types';
...
const ScriptListPanel = forwardRef<{
    ...
    const loadScripts = async () => {
        ...
        try {
            ...
            // 按名称排序
            const sorted = (result || []).sort((a: ScriptData, b: ScriptData) =>
    ...
    const handleExport = async (scriptId: string) => {
        if (!runtime.exportScript) return;
        try {
            await runtime.exportScript(scriptId);
        } catch (err: any) {
            if (err.message) {
    ...
    const handleVarSubmit = async (values: Record<string, string>) => {
        ...
        try {
            await runtime.replayWithVars(scriptId, activeSessionId, values);
        } catch (err: any) {
    ...
    const handleCreate = async () => {
        ...
        setCreating(true);
        try {
            await runtime.create(name, newDesc.trim());
            setShowCreate(false);
            setNewName('');
            setNewDesc('');
            await loadScripts();
