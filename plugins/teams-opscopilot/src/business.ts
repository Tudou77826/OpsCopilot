...
export class OpsBusiness {
  ...
  private async dispatch(operation: string, payload: ObjectValue, generation: number): Promise<unknown> {
    ...
    switch (operation) {
      ...
      case 'connections.folder': return rpc('shell.configs.createFolder', { name: text(payload.name, 128), parentId: payload.parentId ? id(payload.parentId) : '' })
      case 'connections.move': return rpc('shell.configs.moveNode', { id: id(payload.id), newParentId: payload.parentId ? id(payload.parentId) : '', index: integer(payload.index, 0, 100000) })
      case 'connections.reorder': {
        if (!Array.isArray(payload.orderedIds) || payload.orderedIds.length > 1000) throw new OpsError('INVALID_ARGUMENT', '排序无效')
        return rpc('shell.configs.reorderNodes', { parentId: payload.parentId ? id(payload.parentId) : '', orderedIds: payload.orderedIds.map(value => id(value)) })
      }
      case 'connections.duplicate': return rpc('shell.configs.duplicateConnection', { id: id(payload.id) })
