...
function App() {
    ...
    const handleDuplicateTerminal = (id: string) => {
        ...
        // 复制标签 = 用同一份配置再开一个新会话（后端按已存配置重新拨号）。
        // 注意入参是运行态会话 ID，与已保存连接树无关——复制已保存连接走会话树右键菜单。
        if (window.go?.main?.App?.DuplicateTerminalSession) {
            window.go.main.App.DuplicateTerminalSession(id).then((result: main.ConnectResult) => {
                if (result.success) {
                    const newTerminal: TerminalSession = {
                        id: result.sessionId,
