...
func TestSharedSessionMutationPreservesCredentialsAndExternalEdits(t *testing.T) {
	...
	if _, e = desktop.CreateFolder("outside", ""); e != nil {
		t.Fatal(e)
	}
	nodes, e := plugin.List()
	if e != nil {
	...
	if e = plugin.UpdateConnection(nodes[0].ID, ConnectionInput{Host: "fixture", Port: 22, User: "test", HostKey: "pin"}); e != nil {
		t.Fatal(e)
	}
	desktop.Load()
	nodes2 := desktop.Snapshot()
	if len(nodes2) != 2 {
